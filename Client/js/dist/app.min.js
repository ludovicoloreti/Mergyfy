...
app.controller("GetEventCtrl",["$rootScope","$scope","$http","$window","Evento",function(o,t,e,n,a){console.log(a.id),obj={},data={},obj.action="getEvent",data.event_id=parseInt(a.id),data.user_id=parseInt(window.localStorage.id),obj.data=data,console.log(JSON.stringify(obj)),e.post(o.url,[obj]).success(function(o){for(console.log(o[0].data),ress=o[0].data,i=0;i<ress.length;i++)t.evento=ress[i]}).error(function(o){console.log(o,"non vaaaa")}),t.openMap=function(o,t){var e="http://maps.apple.com/maps?q="+t.latitude+","+t.longitude;n.location.href=e,n.open(e,"_system","location=no")}}]);
